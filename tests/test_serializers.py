...
from zeitlabs_payments.serializers import CourseSerializer, CartItemSerializer, CartSerializer
...
def test_course_serializer_course_image_exception(mock_relative_url):
    ...
    mock_relative_url.side_effect = Exception('something went wrong')
...
@pytest.mark.django_db
def test_cart_item_serializer_for_paid_course_type_and_invalid_ref_id(base_data):
...
@pytest.mark.django_db
def test_cart_serializer_returns_serialized_items(base_data):
    user = get_user_model().objects.get(id=3)
